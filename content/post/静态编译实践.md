...
tags: ["GraalVM", "Spring Boot"]
keywords: ["GraalVM", "静态编译", "Spring Boot"]
draft: false
---
...
{{% admonition warning %}}
 ...
 打包时遇到上述错误，原因是：没有安装native-image。
...
1. 下载Github的ca证书。
   ...
   ![我裂开了](/image/静态编译/1.png)

   ![我裂开了](/image/静态编译/2.png)

   ![我裂开了](/image/静态编译/3.png)
...
截止到2022.11.28。项目使用到的不兼容的框架：

1. Druid

   与Spring Boot 3.0不兼容，原因是javax包变改为jakarta（oracle在19年把javaee捐给eclipse基金会）。

   Github上并不活跃，所以兼容版本什么时候能出还是个未知数，所以数据库连接池换成了HikariCP。

2. Mybatis

   2022.11.25 Spring Boot 3.0发布时不兼容，2022.11.28已经兼容。但是静态编译的支持还在进行中。

   相关issue：[https://github.com/mybatis/spring-boot-starter/issues/760](https://github.com/mybatis/spring-boot-starter/issues/760)

3. json-lib

   与jdk的类冲突，导致Mybatis启动报错

{{% /admonition %}}

[^1]:《GraalVM与Java静态编译：原理与应用》
[^2]: https://www.graalvm.org/22.0/reference-manual/native-image/MemoryManagement/
[^3]: https://www.graalvm.org/latest/reference-manual/native-image/metadata/AutomaticMetadataCollection/
